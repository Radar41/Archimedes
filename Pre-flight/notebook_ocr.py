...
Output ONLY the transcription. No commentary, no headers, no "Here is the transcription" preamble.
"""
...
def check_ollama_ready(model: str) -> bool:
    ...
    try:
        ...
        # Check for exact match or prefix match (llava matches llava:13b etc.)
...
def transcribe_page(image_path: str, model: str, is_legend: bool = False) -> dict:
    """Send a single image to the vision model and get transcription."""
...
def extract_errors(transcript: str) -> list[str]:
    """Pull out flagged uncertain sections from the transcript."""
    errors = []
...
def process_image(image_path: str, model: str, output_dir: str, is_legend: bool = False):
    """Process a single image: transcribe, save outputs, report."""
    ...
    with open(err_path, "w") as f:
        if errors:
            f.write(f"# Flagged items for {image_path}\n")
            f.write(f"# {len(errors)} items need review\n\n")
            for e in errors:
                f.write(f"{e}\n")
        else:
    ...
    color_tags = sum(1 for l in lines if any(t in l for t in ["[BLACK]", "[BLUE]", "[RED]", "[GREEN]", "[PURPLE]", "[ORANGE]", "[BROWN]", "[PENCIL]", "[UNK_COLOR]"]))

    print(f"  OK — {len(lines)} lines, {color_tags} color-tagged, {len(errors)} flagged")
...
def main():
    parser = argparse.ArgumentParser(description="Transcribe handwritten notebook pages via Ollama vision model")
    parser.add_argument("input", help="Image file or directory of images")
    parser.add_argument("--model", default="llava:13b", help="Ollama vision model (default: llava:13b)")
    parser.add_argument("--output-dir", default="./transcripts", help="Output directory")
    parser.add_argument("--legend", action="store_true", help="Treat input as the color legend page")
    ...
    print(f"  Flagged items: {summary['total_errors']}")
    print(f"  Total time:    {summary['total_time']:.1f}s")
    if summary["success"]:
        print(f"  Avg per page:  {summary['total_time'] / summary['success']:.1f}s")
